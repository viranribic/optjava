...
public class NNetLayerFast {
	 ...
	 */
	public NNetLayerFast(int numberOfNodes, ITransferFunction transFunct) {
	 ...
	 */
	public int calcDifferentEdges(NNetLayerFast nextLayer) {
	 ...
	 */
	public void calcNodeValues(NNetLayerFast prevLayer, ArrayList<ArrayList<Double>> weightInThisLayer) {
	...
	public void calcNodeValues(NNetLayerFast prevLayer, NNetLayerFast contextLayer,
	...
	public int calcNodeValues(NNetLayerFast prevLayer, double[] weights,int weightPos) {
		...
		for(int i=0;i<layerNodes.size();i++){
			...
			for(int j=0;j<prevLayer.nodeCount();j++){
				NNNode preLayerNode=prevLayer.layerNodes.get(j);
				net+=preLayerNode.getNet()*weights[weightPos++];
			}
			net+=weights[weightPos++];
